...
public class Trie <V>
  ...
  private Entry<V> find (String key, int iKey, List<Entry<V>> list) {
    ...
    // must match.  Increment both iKey and iSub until that is not true.
    ///
  ...
  private V put (String key, int iKey, V value, List<Entry<V>> list) {
    ...
    if(iEntry == list.size() || list.get(iEntry).sub.charAt(0) > key.charAt(iKey))
	{
		Entry<V> newEntry = new Entry(key.substring(iKey), key, value);
    ...
    ///
//key = bottle
    //use substring method... node.sub.substring(iSub)
    //get the unmatched and set its key and value first then take the matched part 
    //and set into the entry.... node.sub.substring(0, iSub)
    //need to create a new list and add b bob 5 into it... don't really need a new list
   
    if(iSub < entry.sub.length())
    {
    	Entry<V> entry2 = new Entry(entry.sub.substring(iSub), entry.key, entry.value, entry.list);
    	entry.sub = entry.sub.substring(0, iSub);
    	entry.key = null;
    	entry.value = null;
    	entry.list = new ArrayList<Entry<V>>();
    	entry.list.add(entry2);
    }
  ...
  public V remove (Object keyAsObject) {
    return remove((String)keyAsObject, 0, list);
  }
  ...
  private V remove (String key, int iKey, List<Entry<V>> list) {
	  ...
	  //check if there
	  Entry<V> entry = find(key, iKey, list);
	  if(entry.key == null)
	  {
		  return null;
	  }
